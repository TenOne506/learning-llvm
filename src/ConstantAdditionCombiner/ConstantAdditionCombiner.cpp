...
namespace {
/**
 * An analysis to collect all the "add" instructions with constant operands from
 * a function
 */
struct ConstantAdditionAnalysis
    : public llvm::AnalysisInfoMixin<ConstantAdditionAnalysis> {
  using Result = llvm::SmallVector<llvm::BinaryOperator *, 0>;

  Result run(llvm::Function &function,
...
ConstantAdditionAnalysis::Result
ConstantAdditionAnalysis::run(Function &function,
...
/**
 * An pass to print all the "add" instructions with constant operands from a
 * function
 */
struct ConstantAdditionPrinterPass
    : public llvm::PassInfoMixin<ConstantAdditionPrinterPass> {
  explicit ConstantAdditionPrinterPass(llvm::raw_ostream &outStream)
      : out(outStream) {}

  llvm::PreservedAnalyses run(llvm::Function &function,
                              llvm::FunctionAnalysisManager &analysisManager) {
    auto &addInstructions =
        analysisManager.getResult<ConstantAdditionAnalysis>(function);
...
struct ConstantAdditionCombinerPass
    : public llvm::PassInfoMixin<ConstantAdditionCombinerPass> {
  llvm::PreservedAnalyses run(llvm::Function &function,
                              llvm::FunctionAnalysisManager &analysisManager);
};
...
PreservedAnalyses
ConstantAdditionCombinerPass::run(Function &function,
                                  FunctionAnalysisManager &analysisManager) {
  auto &addInstructions =
      analysisManager.getResult<ConstantAdditionAnalysis>(function);
...
extern "C" PassPluginLibraryInfo LLVM_ATTRIBUTE_WEAK llvmGetPassPluginInfo() {
  return {LLVM_PLUGIN_API_VERSION, "ConstantAdditionCombiner", "v0.1",
